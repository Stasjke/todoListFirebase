...
import { useTodoApi, useTodoFilters, useModal } from './hooks';

function App() {
	const [todos, setTodos] = useState([]);
	const [isLoading, setIsLoading] = useState(false);

	const { addTodo, deleteTodo, editTodo } = useTodoApi(todos, setTodos, setIsLoading);
	const {
		sortTodosOn,
		searchQuery,
		todosSort,
	...
	const handleEditCase = () => {
		editTodo(selectedTodoId, editTitle).then(() => {
			setModalEditCase(false);
			setSelectedTodoId(null);
			setEditTitle('');
		});
		...
		<>
			<div className={styles.appContainer}>
				...
				<AddTodo
					...
					todosSort={todosSort}
					sortTodosOn={sortTodosOn}
					setSortTodosOn={setSortTodosOn}
				/>
				...
				{modalAddCase && (
					<ModalPanel
						...
						setModalAddCase={setModalAddCase}
				...
				{modalEditCase && (
					<EditModal
						...
						setModalEditCase={setModalEditCase}
